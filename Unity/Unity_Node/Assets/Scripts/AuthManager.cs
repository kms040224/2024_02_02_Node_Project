...
{
    private const string SERVER_URL = "https://localhost:4000";
    ...
    public IEnumerator RefreshToken()
    {
        ...
        {
            Debug.LogError("리프레시 토큰이 없으므로 다시 login ");
            yield break;
        }

        var refreshData = new { refreshToken };
        var jsonData = JsonConvert.SerializeObject(refreshData);
        using (UnityWebRequest www = new UnityWebRequest.PostWwwForm($"{SERVER_URL}/token", "POST"))
        {
            byte[] bodyRaw = Encoding.UTF8.GetBytes(jsonData);
            www.uploadHandler = new UploadHandlerRaw(bodyRaw);
            www.downloadHandler = new DownloadHandlerBuffer();
    ...
    public IEnumerator GetProtectedData()
    {
        ...
        {
            www.SetRequestHeader("Autherization", $"Bearer {accessToken}");
