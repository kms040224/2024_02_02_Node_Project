...
//퀘스트 목록 조회
app.get('/quest/:playerId' , async (req, res) => {
    const {playerId, questId, status} = req.body;
    try
    {
        const[quests] = await pool.query(
            'SELECT q.* , pq.status FROM player_quests pq JOIN quests q ON pq.quests_id = q.quest_id WHERE pq.player_id = ?',
            [req.params.playerId]
        );
        res.json(quests);
    }
...
app.get('quests/status' , async (req, res) => {
    ...
    {
        await pool.query(
            'UPDATE player_quests SET status = ? , complete_at = IF(? = "완료", CURRENT_TIMESTAMP, null) WHERE player_id = ? ANS quest_id = ?',
